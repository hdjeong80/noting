...
import 'layout.dart';
import 'speed_dial_controller.dart';
    ...
    return AnchoredOverlay(
      ...
      overlayBuilder: (context, offset) {
        return AnimatedOpacity(
          opacity: (context.watch<AppData>().isHistoryScreen ||
                  context.watch<AppData>().isPopupScreen)
              ? 0.0
              : 1.0,
          duration: (context.watch<AppData>().isHistoryScreen ||
                  context.watch<AppData>().isPopupScreen)
              ? Duration(milliseconds: 100)
              : Duration(milliseconds: 500),
          curve: (context.watch<AppData>().isHistoryScreen ||
                  context.watch<AppData>().isPopupScreen)
              ? Curves.easeOutExpo
              : Curves.easeInExpo,
          child: context.watch<AppData>().isPopupScreen
              ? Container()
              : SpeedDial(
    ...
    return FadeTransition(
      ...
      child: widget.actions[index].label != null
          ? Container(
              ...
              margin: EdgeInsets.only(right: 2.0, bottom: 10),
